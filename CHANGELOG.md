...
- **平台优先识别**：支持 Ollama、LM Studio、ComfyUI、HuggingFace Cache 和用户自定义目录；Ollama 解析 manifest 展示真实模型名，并按 blob 映射统计占用。
- **深度发现兜底**：默认使用配置文件与已知目录快速扫描；开启“深度发现”后追加 MFT 特征扫描本地 NTFS 盘，按模型文件扩展名与体积阈值筛选候选，并跳过已覆盖路径避免重复计数。
- **结果视图收敛**：概览视图改为平台占用饼图和模型类型柱状图，模型列表支持按平台、类型和排序筛选，保留打开目录与 Bing 搜索模型信息。
...
- **页面模式性能优化**：页面模式下非当前模块会保留本地状态但跳过大列表/图表 DOM 渲染，减少多个模块已有扫描结果后切换菜单的卡顿。
